...
function App() {
  ...
  if (!session) return <AuthGateway onAuth={completeAuth} />;
...
function AuthGateway({ onAuth }) { const [mode, setMode] = useState('login'); const [form, setForm] = useState({ fullName: '', email: '', phoneNumber: '', country: 'NG', password: '' }); const [error, setError] = useState(''); const [loading, setLoading] = useState(false); const update = (event) => setForm({ ...form, [event.target.name]: event.target.value }); const submit = async (event) => { event.preventDefault(); setLoading(true); setError(''); try { const body = mode === 'login' ? { email: form.email, password: form.password } : form; const response = await fetch(`${API_BASE}/auth/${mode === 'login' ? 'login' : 'register'}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await response.json(); if (!response.ok) throw new Error(data.message || 'Unable to authenticate'); onAuth(data); } catch (problem) { setError(problem.message || 'Network error'); } finally { setLoading(false); } }; return <main className="auth-shell"><section className="auth-card"><div className="auth-brand"><div className="auth-logo">W</div><span>WalletApp</span></div><div className="auth-copy"><h1>{mode === 'login' ? 'Welcome back' : 'Create your account'}</h1><p>{mode === 'login' ? 'Log in to manage your digital assets securely.' : 'Set up your secure wallet in a few moments.'}</p></div><div className="auth-methods"><button className="method active">Email</button><button className="method" type="button" onClick={() => setError('Phone OTP sign-in is not connected yet.')}>Phone</button></div><form className="gateway-form" onSubmit={submit}>{mode === 'register' && <><label>Full name<input name="fullName" placeholder="Enter your full name" value={form.fullName} onChange={update} required /></label><label>Phone number<input name="phoneNumber" placeholder="+234 801 234 5678" value={form.phoneNumber} onChange={update} required /></label></>}<label>Email<input name="email" type="email" placeholder="Enter your email" value={form.email} onChange={update} required /></label><label>Password<input name="password" type="password" placeholder="Enter your password" minLength="12" value={form.password} onChange={update} required /></label>{mode === 'login' && <button type="button" className="forgot" onClick={() => setError('Password reset is not connected yet.')}>Forgot password?</button>}{error && <p className="auth-error">{error}</p>}<button className="gateway-submit" disabled={loading}>{loading ? 'Please wait…' : mode === 'login' ? 'Log in' : 'Create account'}</button></form><div className="divider"><span>or continue with</span></div><div className="social-row"><button type="button" onClick={() => setError('Google sign-in is not connected yet.')}>G <span>Google</span></button><button type="button" onClick={() => setError('Apple sign-in is not connected yet.')}>● <span>Apple</span></button></div><p className="auth-switch">{mode === 'login' ? 'New to WalletApp?' : 'Already have an account?'} <button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }}>{mode === 'login' ? 'Sign up' : 'Log in'}</button></p><p className="auth-legal">By continuing, you agree to the WalletApp Terms of Service and Privacy Policy.</p></section><style>{`html,body,#root{width:100%;height:100%;overflow:auto}.auth-shell{min-height:100dvh;display:grid;place-items:center;padding:max(24px,env(safe-area-inset-top)) 18px max(24px,env(safe-area-inset-bottom));background:radial-gradient(ellipse at 80% 0,#30230c 0,transparent 36%),#0d0f12}.auth-card{width:min(100%,430px);padding:28px 24px;background:#17191e;border:1px solid #2b2e34;border-radius:20px;box-shadow:0 22px 70px #0008}.auth-brand{display:flex;align-items:center;gap:9px;font-weight:700;font-size:17px}.auth-logo{display:grid;place-items:center;width:32px;height:32px;border-radius:10px;background:#f7a600;color:#17120a;font-weight:800}.auth-copy h1{font:700 28px 'Space Grotesk';margin:29px 0 7px}.auth-copy p,.auth-legal{color:#8f96a3;font-size:12px;line-height:1.5}.auth-methods{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:24px 0 17px}.method{padding:10px;border-radius:8px;background:#111317;color:#9ba1ab;border:1px solid #292d34;font-size:12px}.method.active{color:#f7a600;border-color:#f7a600;background:#2a210e}.gateway-form{display:grid;gap:13px}.gateway-form label{display:grid;gap:6px;color:#bac0c8;font-size:11px}.gateway-form input{width:100%;padding:13px 12px;border:1px solid #30343b;border-radius:9px;background:#101216;color:#f5f5f5;outline:0}.gateway-form input:focus{border-color:#f7a600}.forgot{justify-self:end;color:#f7a600;font-size:11px;margin-top:-4px}.gateway-submit{margin-top:4px;width:100%;padding:13px;border-radius:9px;background:#f7a600;color:#1b150a;font-weight:700}.gateway-submit:disabled{opacity:.5}.divider{display:flex;align-items:center;gap:10px;margin:21px 0;color:#737b86;font-size:10px}.divider:before,.divider:after{content:'';height:1px;flex:1;background:#2c3036}.social-row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.social-row button{padding:11px;border:1px solid #30343b;border-radius:9px;background:#1b1d22;color:#eef0f2;font-size:12px}.auth-switch{text-align:center;margin:23px 0 13px;color:#9ba1ab;font-size:12px}.auth-switch button{color:#f7a600;font-weight:700}.auth-legal{text-align:center;margin:0;font-size:10px}@media(min-width:481px){.auth-card{border-radius:24px}}`}</style></main>; }
